...
parse arg x highV bot .                /*obtain any C.L. specifications.*/
...
if x<1  then exit                      /*if  x ≤ 0, then we're all done.*/
  do m=1 for  bot;  r=ffr(m); both.r=1 /*calculate 1st   40  R   values.*/
  end   /*m*/                          /* [↑]  build first  40 R values.*/
        do n=1 for highV-bot; s=ffs(n) /*calculate 1st  960  S   values.*/
...
/*──────────────────────────────────SAYERR subroutine───────────────────*/
sayErr:  errs=errs+1;    say  '***error***!'  arg(1);     return
