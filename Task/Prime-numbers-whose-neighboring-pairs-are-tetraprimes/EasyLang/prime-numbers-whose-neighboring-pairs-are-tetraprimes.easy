...
proc calc max dir .
   ...
   while prim < max
      if nfac[prim + dir] = 4 and nfac[prim + dir + dir] = 4
         if (prim + dir) mod 7 = 0 or (prim + dir + dir) mod 7 = 0 : cnt7 += 1
   ...
   if max <= 100000 : print ""
   print ""
   print "Count: " & len d[] + 1 & " -  Count with factor 7: " & cnt7
   print ""
   sort d[]
   ld = len d[]
   med = d[ld div 2 + 1]
   if ld mod 2 = 0 : med = (med + d[ld div 2]) / 2
   print "Gaps: min: " & d[1] & "  median: " & med & "  max: " & d[ld]
   print ""
.
calc 100000 -1
calc 100000 1
