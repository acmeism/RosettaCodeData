...
mat.  =
mat.1 =   '    1   2   -1      -4   '  /*a few extra blanks can't hurt. */
mat.2 =   '    2   3   -1     -11   '
mat.3 =   '   -2   0   -3      22   '
  ...
  cols = max(cols, c)                  /*remember the max number of cols*/
  end   /*r*/

rows =    r - 1                        /*adjust the row count (from DO).*/
...
/*═══════════════════════════════════Reduced Row Echelon Form on matrix.*/
  ...
  ?=@.r.!
          ...
          do k=1  for rows;  ?=@.k.!   /*sub (row K) *@.r.s  from row K */
          if k==r | ?=0 then iterate   /*skip if row k is the same as R */
                      do s=1  for cols /*perform for the rest of the row*/
                      @.k.s=@.k.s  -  ? * @.r.s
                      end   /*s*/
          end   /*k*/
  !=!+1
