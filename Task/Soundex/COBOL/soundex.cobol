...
Procedure division.
    Perform varying wrd-idx from 1 by 1 until wrd-idx greater than 28
        call "soundex-encode" using
            by content sample-word(wrd-idx)
...
Encode-start.
    ...
    Call "soundex-char" using
        by content str-first-let
        by reference let-code.
    Move let-code to prv-let-code.

Encode-string.
    Perform varying str-idx from 1 by 1
        until str-idx greater than 15 or sdx-idx greater than 3
        call "soundex-char" using
            by content str-rest-let(str-idx)
            by reference let-code
        if let-code not equal 7 then
            if let-code not equal 0 and let-code not equal prv-let-code then
