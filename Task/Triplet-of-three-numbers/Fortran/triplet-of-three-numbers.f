...
integer(kind=tInt), parameter ::limit = 6005_tInt                 ! largest prime number we need
! PrimeTriplet uses: integer(kind=tInt), parameter ::limit = 5505_tInt            ! largest p+6 if  prime p<5500
...
write (*, '("   n    n-1  n+3  n+5",/,"---------------------")')
! PrimeTriplet uses: write (*, '("    p  p+2  p+6 ",/,"-----------------")')
do ii=1,  highPrimes-2                                            ! so that primes(ii+2) can be addressed
  if (      primes(ii+1) .eq. primes(ii)+4 &                      ! next prime is 4 larger than current one
      .and. primes(ii+2) .eq. primes(ii)+6  ) then                ! and second next is 6 larger: n = primes(ii)+1
    write (*, '(i4,": ", 3i5)' )  primes(ii)+1, primes(ii), primes(ii+1), primes(ii+2)
! PrimeTriplet uses:   if (      primes(ii+1) .eq. primes(ii)+2 &                      ! next prime is 2 larger than current one
! PrimeTriplet uses:       .and. primes(ii+2) .eq. primes(ii)+6  ) then                ! and second next is 6 larger: n = primes(ii)+1
! PrimeTriplet uses:     write (*, '(3i5)' )   primes(ii), primes(ii+1), primes(ii+2)
  endif
